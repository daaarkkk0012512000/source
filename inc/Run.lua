...
venom = Token:match("(%d+)")
redis:set(venom..":VERSION",1)
redis:set(venom..":SUDO_ID:",SUDO_USER)
redis:set(venom..":DataCenter:",'German')
redis:set(venom..":UserNameBot:",BOT_User)
redis:set(venom..":NameBot:",BOT_NAME)
redis:hset(venom..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3venom_INSTALL","Yes")
info = {}
...
VENOM ✾ @DA_RKK
│
تم تطوير وبرمجة السورس من قبل داركᐂ .
│@DA_RKK
-------------------------------------------------------------------
]]..'\027[0;32m')
create_config() 
...
venom = Token:match("(%d+)")
our_id = tonumber(venom)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(venom..":UserNameBot:")
SUDO_ID = tonumber(redis:get(venom..":SUDO_ID:"))
SUDO_USER = redis:hgetall(venom..'username:'..SUDO_ID).username
version = redis:get(venom..":VERSION")
DataCenter = redis:get(venom..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..venom..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(venom..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(venom..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(venom..'group:add'..msg.chat_id_) then 
msg.GroupActive = true
else
msg.GroupActive = false
end

if msg.sender_user_id_ == 1282382975 then 
msg.TheRank =  'دارك'
msg.Rank = 1
elseif msg.sender_user_id_ == 1596391910 then 
msg.TheRank =  'بيكاسو'
msg.Rank = 1
...
msg.TheRankCmd = 'فينوم' 
msg.TheRank = 'مطور اساسي' 
msg.Rank = 1
elseif redis:sismember(venom..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'فينوم'
msg.TheRank = 'مطور البوت'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(venom..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(venom..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(venom..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(venom..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(venom..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(venom..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(venom..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(venom..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(venom..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(venom..':WELCOME_BOT'),[[▪️ مرحباً انا بوت ]]..redis:get(venom..':NameBot:')..[[ .
▪️ اختـصاصـي حمايـة الـمجموعات
▪️ تأكد من رفعـك للبـوت مشـرف في مجموعـتك واكتب تفعـيل 
قناة سورس فينوم ❥ @Sourcevenom
...
if msg.GroupActive and redis:get(venom..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(venom..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(venom..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(venom..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.venom then 
if msg.text and ScriptFile.ivenom then
for k, venom in pairs(ScriptFile.venom) do
local SearchText = msg.text:match(venom)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.ivenom(msg,{msg.text:match(venom)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",venom.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dvenom then
if ScriptFile.dvenom(msg) == false then
return false
end
...
if Plug.venom then 
if msg.text and Plug.ivenom then
for k, venom in pairs(Plug.venom) do
local SearchText = msg.text:match(venom)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(venom..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(venom..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.ivenom(msg,{msg.text:match(venom)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",venom.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source venom")
end 

end
else
print("The File Script.lua Not Runing in The Source venom")
...
function tdcli_update_callback(data)
	...
	redis:srem(venom..'users',msg.sender_user_id_)
	end
	...
	EditMsg(data.message_.chat_id_,data.message_.id_,'10% - |V          |')
	EditMsg(data.message_.chat_id_,data.message_.id_,'20% - |V         |')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/venom2/main/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |V E       |')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/venom2/main/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |V E      |')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/venom2/main/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |V E N   |')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/venom2/main/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/venom2/main/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/venom2/main/plugins/games.lua','./plugins/games.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'100% - |V E N O M|\n\n▪️ ** السورس الى اصدار \n▪️ ** تم اعاده تشغيل السورس بنجاح')
	dofile("./inc/Run.lua")
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	return sendMsg(msg.chat_id_,msg.id_," ▪️ تم تحديث الملفات .\n",nil,function(arg,data)
	Refresh_Start = true
	end)
	end 
	...
	download_file('https://raw.githubusercontent.com/jack1eljoker1/venom2/main/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/venom2/main/plugins/games.lua','./plugins/games.lua')
	sendMsg(msg.chat_id_,msg.id_,'▪️ {* تــم تحديث وتثبيت السورس  *} .\n\n▪️ { Bot is Update » }',nil,function(arg,data)
	dofile("./inc/Run.lua")
	...
	mmd = redis:get(venom..":SUDO_ID:")
redis:hset(venom..'username:'..tonumber(mmd),'username',msg.text)
	send_msg(msg.chat_id_,"▪️ عزيزي تم تغيير المطور الاساسي بنجاح الان ارسل reload ...")
	end
	...
	redis:set(venom..":SUDO_ID:",msg.text)
send_msg(msg.chat_id_,"▪️ تم تثبيت الايدي الان قم برسال معرف المطور @UserName ...")
	end
	if msg.text== 'تغير المطور الاساسي' and msg.sender_user_id_ == SUDO_ID then
    send_msg(msg.chat_id_,"▪️ عزيزي قم برسال ايدي المطور ...")
redis:setex('setid'..msg.sender_user_id_,120,true)
end
	
	
	if msg.text== 'reload' and msg.sender_user_id_ == SUDO_ID then
	sendMsg(msg.chat_id_,msg.id_,'▪️ {* تــم أعـاده تشغيل البوت  *} .\n\n▪️ { Bot is Reloaded » }.',nil,function(arg,data)
	dofile("./inc/Run.lua")
	print("Reload ~ ./inc/Run.lua")
	end)
	return false
...
download(stk,32)
	...
	if redis:get(venom..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(venom..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(venom..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(venom..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(venom..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(venom..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(venom..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(venom..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(venom..'group:add'..data.chat_.id_) then
	redis:set(venom..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(venom..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(venom..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(venom..'group:name-100'..data.channel_.id_) or "")
	send_msg(SUDO_ID,"▪️ قام شخص بطرد البوت من المجموعه الاتيه : \n▪️ ألايدي : `-100"..data.channel_.id_.."`\n▪️ الـمجموعه : "..Flter_Markdown(NameGroup).."\n\n▪️ تـم مسح كل بيانات المجموعه بنـجاح ")
	...
	redis:mset(
	venom..'group:name'..k,IDS.Title,
	venom..'num_msg_venom'..k,5,
	venom..'group:add'..k,true,
	venom..'lock_link'..k,true,
	venom..'lock_spam'..k,true,
	venom..'lock_webpage'..k,true,
	venom..'lock_markdown'..k,true,
	venom..'lock_flood'..k,true,
	venom..'lock_bots'..k,true,
	venom..'mute_forward'..k,true,
	venom..'mute_contact'..k,true,
	venom..'mute_document'..k,true,
	venom..'mute_inline'..k,true,
	venom..'lock_username'..k,true,
	venom..'replay'..k,true
	)
	redis:sadd(venom..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(venom..'username:'..ID,'username',user)
	redis:sadd(venom..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(venom..'username:'..ID,'username',user)
	redis:sadd(venom..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(venom..'username:'..ID,'username',user)
	redis:sadd(venom..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'▪️ ** تم رفع النسخه‏‏ الاحتياطيه\n▪️ ** حالياً عدد مجموعاتك هيه‏ *'..redis:scard(venom..'group:ids')..' .*\n')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(venom..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(venom..'username:'..data.user_.id_).username
	...
	redis:hset(venom..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
